...
async function calcSignal(b,ax,sym,layer){
  ...
  var kl=await ax.getKlines(sym,cfg2.tf,150);
  ...
  log('INFO',sym+' ['+layer+'] BS:'+bs+' SS:'+ss+' ['+rsn.join('+')+'] -> '+signal,b);
  return{signal:signal,bs:bs,ss:ss,price:last};
}
...
async function tradingLoop(b){
  ...
  if(b.learningPause){await checkPositions(b);return;}
  var ax=api(b);
  try{
    ...
    for(var i=0;i<b.cfg.symbols.length;i++){
      ...
      for(var layerName in LAYERS){
        try{
          ...
          if(b.openTrades[sym+'_'+layerName+'_L']||b.openTrades[sym+'_'+layerName+'_S'])continue;
          ...
          var slPct=Math.max(MIN_SL,layerCfg.sl),tpPct=Math.max(slPct*MIN_RR,layerCfg.tp);
          var slP=+(res.signal==='BUY'?cur-cur*slPct/100:cur+cur*slPct/100).toFixed(4);
          var tpP=+(res.signal==='BUY'?cur+cur*tpPct/100:cur-cur*tpPct/100).toFixed(4);
